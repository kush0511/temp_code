...
from kombu.transport import redis, virtual
...
from ai_bus_commons.celery import redis_cluster_transport as redis_cluster  # noqa
...
class Client:
    ...
    def __init__(self, db=None, port=None, connection_pool=None, **kwargs):
        ...
        self.startup_nodes = [self.ClusterNode(self), self.ClusterNode(self)]
    ...
    def zadd(self, key, *args):
        if redis_cluster.redis.VERSION[0] >= 3:
            (mapping,) = args
            for item in mapping:
    ...
    def get_nodes(self):
        return self.startup_nodes
    
    def get_redis_connection(self, node=None):
        return self.startup_nodes[0].connection
    ...
    def pubsub(self, *args, **kwargs):
        connection = self.startup_nodes[1].connection
        ...
        self.pubsub_connection = connection
...
class Transport(redis_cluster.RedisClusterTransport):
    Channel = Channel
...
class test_Channel:
    ...
    def test_subscribe(self):
        ...
        assert sorted(s_args[0]) == ['/default/_kombu/fanout.a', '/default/_kombu/fanout.b']
    ...
    def test_receive_connection_has_gone(self):
        ...
        self.channel.subclient.set_pubsub_node = Mock()

        assert self.channel._receive(**options)
        assert _receive_one.called

    def test_brpop_read_raises(self):
        c = self.channel.client = MagicMock()
        node = c.get_node.return_value = Mock()
        node.redis_connection.parse_response.side_effect = KeyError('foo')
        connection = c.startup_nodes[0].connection
    ...
    def test_brpop_read_gives_None(self):
        c = self.channel.client = MagicMock()
        node = c.get_node.return_value = Mock()
        node.redis_connection.parse_response.return_value = None
        connection = c.startup_nodes[0].connection
        options = {'conn': connection}

        with pytest.raises(redis_cluster.Empty):
            self.channel._brpop_read(**options)
    ...
    def test_poll_error(self):
        ...
        self.channel._poll_error('BRPOP', **options)

        c.parse_response.assert_called_with(connection, 'BRPOP')
    ...
    def test_put_fanout(self):
        ...
        c().publish.assert_called_with('/default/_kombu/fanout.exchange', dumps(body))
    ...
    def test_delete(self):
        ...
        delete.assert_has_calls([
            call(x._q_for_pri('queue', pri)) for pri in redis_cluster.Channel.priority_steps
        ])
    ...
    def test_has_queue(self):
        ...
        exists.assert_has_calls([
            ...
            for pri in redis_cluster.Channel.priority_steps
        ])
    ...
    def test_close_client_close_raises(self):
        c = self.channel.client = MagicMock()
        connection = c.get_redis_connection()
        node = Mock()
        c.get_nodes.return_value = [node]
        self.channel.brpop_conn = {connection}
    ...
    #     with patch('kombu.transport.redis_cluster.Channel._create_client'):
    ...
    #Tests StrictRedis, is it really needed?
    # def test_get_client(self):
    #     import redis as R
    #     KombuRedisCluster = redis_cluster.Channel._get_client(self.channel)
    #     assert isinstance(KombuRedisCluster, R.cluster.RedisCluster)

    #     Rv = getattr(R, 'VERSION', None)
    #     try:
    #         R.VERSION = (2, 4, 0)
    #         with pytest.raises(VersionMismatch):
    #             redis_cluster.Channel._get_client(self.channel)
    #     finally:
    #         if Rv is not None:
    #             R.VERSION = Rv
    ...
    #     PrefixedRedis = redis_cluster.Channel._get_client(self.channel)
    #     assert isinstance(PrefixedRedis(), PrefixedStrictRedis)

    def test_get_response_error(self):
        from redis.exceptions import ResponseError
        assert redis_cluster.Channel._get_response_error(self.channel) is ResponseError
    ...
    def test_register_with_event_loop(self):
        ...
        loop = Mock(name='loop')
        redis_cluster.RedisClusterTransport.register_with_event_loop(transport, conn, loop)
    ...
    def test_transport_on_readable(self):
        ...
        redis_cluster.RedisClusterTransport.on_readable(transport, 13)
        cycle.on_readable.assert_called_with(13)

    def test_transport_connection_errors(self):
        """Ensure connection_errors are populated."""
        assert redis_cluster.RedisClusterTransport.connection_errors

    def test_transport_channel_errors(self):
        """Ensure connection_errors are populated."""
        assert redis_cluster.RedisClusterTransport.channel_errors

    def test_transport_driver_version(self):
        assert redis_cluster.RedisClusterTransport.driver_version(self.connection.transport)
    ...
    #                 redis_cluster.redis.UnixDomainSocketConnection,
    #             )
    ...
    #             assert issubclass(
    #                 connparams['connection_class'],
    #                 redis_cluster.redis.SSLConnection,
    #             )
    ...
    # @patch("redis.client.ClusterPubSub.execute_command")
...
class test_Redis:
    ...
    @pytest.mark.replace_module_value(redis_cluster.redis, 'VERSION', [3, 0, 0])
    ...
    @pytest.mark.replace_module_value(redis_cluster.redis, 'VERSION', [2, 5, 10])
    ...
    def test_close_poller_not_active(self):
        ...
        c.client.startup_nodes[0].connection
        c.close()
    ...
    def test_close_disconnects(self):
        c = Connection(transport=Transport).channel()
        print(c, c.client, c.subclient)
        conn1 = c.client.startup_nodes[0].connection
        conn2 = c.subclient.connection_pool.get_connection()
        c.brpop_conn = {conn1}
        c.listen_conn = {conn2}
        c.close()
...
class test_MultiChannelPoller:
    ...
    def setup(self):
        self.Poller = redis_cluster.ClusterPoller
    ...
    def test_handle_event(self):
        ...
        chan._poll_error.assert_called_with('BRPOP', **{'connection': conn})
    ...
    def test_register_when_registered_reregisters(self):
        p = self.Poller()
        p.poller = Mock()
        channel, client, connection, type = MagicMock(), MagicMock(), MagicMock(), MagicMock()
        sock = connection._sock = Mock()
        sock.fileno.return_value = 10

        p._chan_to_sock = {(channel, client, connection, type): 6}
        p._register(channel, client, connection, type)
        p.poller.unregister.assert_called_with(6)
        assert p._fd_to_chan[10] == (channel, connection, type)
        assert p._chan_to_sock[(channel, client, connection, type)] == sock
        ...
        connection.connect.side_effect = after_connected
    ...
    def test_get_conns_for_channel(self):
        p = self.Poller()
        channel = MagicMock(active_queues = [1])
        conn1 = Mock()
        conn2 = Mock()
        conn3 = Mock()
        p._chan_to_sock = [('x','x',conn1,'BRPOP'), 
                           ('x','x',conn2,'LISTEN'), 
                           ('x','x',conn3,'BRPOP')]
        p.conn_to_node_brpop = {('host1', 'port1'): conn1}

        assert p._get_conns_for_channel(channel) == [conn1, conn3]

        p.conn_to_node_brpop = {}
        redis_node = channel.client.get_redis_connection.return_value = MagicMock()
        test_conn = MagicMock(host='localhost', port=7000)
        redis_node.connection_pool.get_connection.return_value = test_conn

        assert p._get_conns_for_channel(channel) == [test_conn]
    
    def test_get_fanout_conns_for_channel(self):
        p = self.Poller()
        channel = MagicMock(active_fanout_queues = [1])
        conn1 = Mock()
        conn2 = Mock()
        conn3 = Mock()
        p._chan_to_sock = [('x','x',conn1,'BRPOP'), 
                           ('x','x',conn2,'LISTEN'), 
                           ('x','x',conn3,'BRPOP')]
        p.conn_to_node_listen = {('host1', 'port1'): conn2}

        assert p._get_fanout_conns_for_channel(channel) == [conn2]

        p.conn_to_node_listen = {}
        channel.subclient.set_pubsub_node = Mock()
        redis_node = channel.subclient.get_redis_connection.return_value = MagicMock()
        test_conn = MagicMock(host='localhost', port=7000)
        redis_node.connection_pool.get_connection.return_value = test_conn

        assert p._get_fanout_conns_for_channel(channel) == [test_conn]

    def test_register_BRPOP(self):
        p = self.Poller()
        channel = MagicMock()
        connection = channel.client.startup_nodes[0].connection
        connection._sock = None
        p._get_conns_for_channel = MagicMock(return_value=[connection])
        p._register = Mock()
        ...
        channel.client.startup_nodes[0].connection._sock = Mock()
        p._chan_to_sock[(channel, channel.client, channel.client.startup_nodes[0].connection, 'BRPOP')] = True
        ...
        new_connection = new_channel.client.startup_nodes[0].connection
        new_connection._sock = Mock()
        p._get_conns_for_channel = MagicMock(return_value=[new_connection])
        p._register_BRPOP(channel)

        assert channel._brpop_start.call_count == 2
        assert p._register.call_count == 2
    ...
    def test_register_LISTEN(self):
        ...
        p._get_fanout_conns_for_channel = MagicMock(return_value=[connection])
        p._register = Mock()
        p.conn_to_node_listen = MagicMock()
        channel.subclient.set_pubsub_node = Mock()
        ...
        new_connection = new_channel.client.startup_nodes[0].connection
        new_connection._sock = Mock()
        p._get_fanout_conns_for_channel = MagicMock(return_value=[new_connection])
        p._register_LISTEN(channel)

        assert p._register.call_count == 2
        assert channel._subscribe.call_count == 2
    ...
    def test_get_no_actions(self):
        p, connection, channel = self.create_get()
    ...
    def test_get_brpop_qos_allow(self):
        p, connection, channel = self.create_get(queues=['a_queue'])
    ...
    def test_get_listen(self):
        p, connection, channel = self.create_get(fanouts=['f_queue'])
    ...
    def test_get_receives_ERR(self):
        p, connection, channel = self.create_get(events=[(1, eventio.ERR)])
        p._fd_to_chan[1] = (channel, connection, 'BRPOP')

        with pytest.raises(redis_cluster.Empty):
            p.get(Mock())

        channel._poll_error.assert_called_with('BRPOP', **{'connection': connection})
...
class test_Mutex:
    ...
    def test_mutex(self, lock_id='xxx'):
        ...
        held = False
        with pytest.raises(redis_cluster.MutexHeld):
            with kombu.transport.redis.Mutex(client, 'foo1', 100):
                held = True
            assert not held
        lock.acquire.assert_called_with(blocking=False)
        client.lock.assert_called_with('foo1', timeout=100)
        ...
        lock.acquire.return_value = True
        lock.release.side_effect = redis_cluster.redis.exceptions.LockNotOwnedError()
        held = False
        with redis_cluster.Mutex(client, 'foo1', 100):
            held = True
        assert held
