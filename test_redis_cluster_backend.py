...
class test_RedisResultConsumer:
    def get_backend(self):
        from celery.backends.redis_cluster import RedisClusterBackend as RedisBackend
...
class basetest_RedisBackend:
    ...
    def get_E_LOST(self):
        from celery.backends.redis_cluster import E_LOST
        return E_LOST
    ...
    def chord_context(self, size=1):
        with patch('celery.backends.redis_cluster.maybe_signature') as ms:
...
class test_RedisBackend(basetest_RedisBackend):
    ...
    @patch('celery.backends.redis_cluster.logger')
    ...
    @patch('celery.backends.redis_cluster.retry_over_time')
